...
/*u32 *fdt_find_node_end(u32 *fdt) {
	u32 depth = 0, val;
	while (depth > 0 || (val = load_be(fdt)) != FDT_END_NODE) {
		if (val == FDT_BEGIN_NODE) {
			...
			fdt += 3 + (load_be(fdt + 1) + 3) / 4;
		}
	}
	return fdt + 1;
}*/

void dump_fdt(const struct fdt_header *fdt) {
	if (read_be32(&fdt->magic) == 0xd00dfeed) {
		...
		const be64 *rsvmap = (be64*)((const char *)fdt + reserved_offset);
		if (rsvmap[1].v) {
		...
		const be32 *struct_seg = (const be32 *)((const char *)fdt + struct_offset), *struct_end = struct_seg + struct_size/4;
