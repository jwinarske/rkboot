...
const char *decode_status_msg[NUM_DECODE_STATUS] = {
#define X(name, msg) msg,
	DEFINE_DECODE_STATUS
#undef X
};

const struct decompressor *formats[] = {
#ifdef HAVE_LZ4
	&lz4_decompressor,
#endif
#ifdef HAVE_GZIP
...
int main(int argc, char **argv) {
	...
	assert(buf);
	const struct decompressor **decomp = formats;
	...
	while (1) {
		...
		case COMPR_PROBE_SIZE_KNOWN:;
			fprintf(stderr, "allocating %zu bytes of decompressor state\n", (*decomp)->state_size);
