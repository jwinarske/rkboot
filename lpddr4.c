...
static void mr_adjust(volatile u32 *pctl, volatile u32 *pi, const struct mr_adjustments *adj, u32 regset, u32 val) {
	...
	u32 pctl_value = (val & pctl_mask) << rs->pctl_shift;
	for_range(i, 0, 2) {
...
static const struct odt_settings lpddr4_odt_settings = {
...
static void lpddr4_get_odt_settings(struct odt_settings *odt, const struct odt_preset *preset) {
	*odt = lpddr4_odt_settings;
...
void lpddr4_modify_config(struct dram_cfg *cfg) {
	...
	set_drive_strength((u32*)phy, pi, &cfg_layout, &odt);
