...
//=== DEPCMD_SET_EP_CFG parameters
// fifonum is only used only for IN endpoints, for which it has to be
// unique and in the first 16, even if the hardware has more TXFIFOs
...
// not ready status: mostly useful for isoc EPs, since it provides the
// frame number. Linux used to use it for control EPs as well, but
...
#define DWC3_DTRB_HWO 1	//< hardware-owned
#define DWC3_DTRB_LAST 2
#define DWC3_DTRB_CHAIN 4
// Continue on Short Packet: Linux sets this on all non-stream EPs
// as well as last TRBs for stream EPs.
